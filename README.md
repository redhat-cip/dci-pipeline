...
`dci-pipeline` will export a `job_info.output` dictionnary with a
`kubeconfig` key. Here is an example on how to use it in a playbook:
```YAML
...
Then to get this `kubeconfig` file from another job, you have to
define an `input` field and a `prev_stages` field to specify the types
or names of the previous jobs to lookup for a corresponding `output`
like this:
...
The path of the `kubeconfig` file will be exported by `dci-pipeline` in
the `kubeconfig_path` variable that is passed to the playbook. Here is
an example on how to use it and support standalone runs with an
