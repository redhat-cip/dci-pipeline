...
def pre_process_stage(stage):
    ...
    for k, v in stage["ansible_envvars"].items():
        if v != "/@junit-tmpdir" and v != "/@tmpdir":
            continue
        stage["ansible_envvars"][k] = tempfile.mkdtemp(prefix="dci-pipeline-tmpdir")
        _meta_tmpdir = v[2:]
        if _meta_tmpdir not in metas:
            metas[_meta_tmpdir] = [stage["ansible_envvars"][k]]
        else:
            metas[_meta_tmpdir].append(stage["ansible_envvars"][k])
    return metas, stage


def post_process_stage(context, stage, metas):
    for k, v in metas.items():
        if k != "junit-tmpdir" and k != "tmpdir":
            continue
        if k == "junit-tmpdir":
            for junit_tmpdir in metas["junit-tmpdir"]:
                upload_junit_files_from_dir(context, stage, junit_tmpdir)
        for dir in metas[k]:
            try:
                shutil.rmtree(dir)
            except OSError as e:
