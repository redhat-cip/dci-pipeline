...
def get_components(context, stage, topic_id, tag=None):
    components = []
    for component_type in stage['components']:
        where_query = 'type:%s%s' % (component_type,
                                     (',tags:%s' % tag) if tag else '')
        if '=' in component_type:
            c_type, c_name = component_type.split('=')
        ...
        if resp.status_code == 200:
            log.info('Got component %s: %s' % (component_type, resp.text))
            if resp.json()['_meta']['count'] > 0:
                components.append(resp.json()['components'][0])
            else:
                log.error('No %s component' % component_type)
        else:
            log.error('Unable to fetch component %s for topic %s: %s' % (component_type,
                                                                         stage['topic'],
                                                                         resp.text))
    return components
