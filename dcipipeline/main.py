...
def run_stage(stage, dci_credentials, data_dir, job_info):
    ...
    dci_ansible_dir = stage.get('dci_ansible_dir', os.path.join(os.path.dirname(TOPDIR), 'dci-ansible'))
    if not os.path.isfile(os.path.join(dci_ansible_dir, 'callback', 'dci.py')):
        log.warning('Unable to find dci.py callback in %s/callback/. Reverting to default.' % dci_ansible_dir)
        envvars = {}
    else:
        envvars = {
