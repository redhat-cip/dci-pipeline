...
def schedule_job(stage, remoteci_context, pipeline_user_context, tag=None, prev_components=None):
    log.info(
    ...
    if prev_components:
        ...
        for comp in components:
            if comp["name"] not in prev_comp_names:
                log.info("Found a different component to retry %s from %s" % (comp["name"], prev_comp_names))
                break
        else:
            log.info("No different components with tag %s. Not restarting the job." % tag)
            return None
