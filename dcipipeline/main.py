...
def schedule_job(
        stage, remoteci_context, pipeline_user_context, tag=None, prev_components=None, previous_job_id=None
):
    log.info(
        "scheduling job %s on topic %s%s previous_job_id=%s"
        % (stage["name"], stage["topic"], " with tag %s" % tag if tag else "", previous_job_id)
    )
...
def run_stages(stage_type, pipeline, config_dir, previous_job_id):
    ...
    for stage in stages:
        ...
        stage["job_info"] = schedule_job(
            stage, dci_remoteci_context, dci_pipeline_user_context, previous_job_id=previous_job_id
        )
...
def main(args=sys.argv):
    ...
    for stage_type in get_types_of_stage(pipeline):
        job_in_errors, stages = run_stages(stage_type, pipeline, config_dir, previous_job_id)
        if job_in_errors != 0:
            log.error(
